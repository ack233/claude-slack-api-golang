...
func Init() error {
	...
	envConfigfile := getEnvConfigPath()

	log.SetFlags(log.Lshortfile | log.LstdFlags) //设置日志行号
	var configfile = pflag.StringP("configfile", "c", envConfigfile, "user -c set Your congfile")
	...
	err = ViperConfig.UnmarshalKey("SLACK", &SlackConfig)
	if err != nil { // Handle errors reading the config file
		log.Fatalf("Fatal error config file: %s \n", err)
	}
	return nil
